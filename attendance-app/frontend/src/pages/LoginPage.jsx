...
const LoginPage = () => {
  ...
  const toast = useToast(); // Fixed: direct access, no destructuring
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-background transition-colors duration-500">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-sky-200/40 dark:bg-sky-500/10 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen animate-pulse duration-[10s]" />
      <div className="absolute bottom-[-10%] right-[-5%] w-[500px] h-[500px] bg-purple-200/40 dark:bg-purple-500/10 rounded-full blur-[100px] mix-blend-multiply dark:mix-blend-screen animate-pulse duration-[8s]" />
        ...
        <div className="glass-card rounded-3xl p-8 border border-border shadow-2xl overflow-hidden">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <button
              ...
              disabled={isLoading}
              className={clsx(
                "btn-primary w-full py-2.5 rounded-xl text-sm font-semibold hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center shadow-lg",
                isLoading && "opacity-70 cursor-not-allowed hover:scale-100"
              )}
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span className="flex items-center">Sign In <ArrowRight className="w-4 h-4 ml-1.5 opacity-70" /></span>}
            </button>
          </form>
